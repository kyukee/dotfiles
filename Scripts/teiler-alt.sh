...
screenshotMenu () {
    ...
    if [[ $rofi_exit -eq 10 ]]; then
        ...
        else maimCmd delay $mode "${filename}" && cd "${img_path}" && teiler_helper --upload image "${filename}"; notify-send -a "teiler" "teiler" "Screenshot saved" && exit
        fi
    elif [[ $rofi_exit -eq 11 ]]; then
        delayPrompt
        if [[ $delay == "0" ]]; then maimCmd nodelay $mode "${filename}" && exit
        else maimCmd delay $mode "${filename}"; notify-send -a "teiler" "teiler" "Screenshot saved"
        fi
...
openHistory () {
    ...
    if [[ $menu == "1 Images" ]]; then qimgv "${img_path}";
    elif [[ $menu == "2 Videos" ]]; then qimgv "${vid_path}";
...
stopRecording () {
    ...
    notify-send -a "teiler" "teiler" "Stopped recording"
    if [[ -z $rate ]]; then
...
ffmpegCmd () {
    ...
    if [[ $1 == "fullscreen" ]]; then
        ...
        notify-send -a "teiler" "teiler" "Screencast started"
    elif [[ $1 == "area" ]]; then
...
elif [[ $1 == "--history" ]]; then openHistory;
elif [[ $1 == "--quick" ]]; then
    if [[ $2 == "image" ]]; then
      check_img_path
